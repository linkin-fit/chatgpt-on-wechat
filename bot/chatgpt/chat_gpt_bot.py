...
class ChatGPTBot(Bot, OpenAIImage):
    ...
    def reply_text(self, session: ChatGPTSession, api_key=None, args=None, retry_count=0) -> dict:
        ...
        try:
            ...
            #}
            #logger.info("[ChatGPT] replying")
            #response = openai.ChatCompletion.create(
            #    model= conf().get("model") or "gpt-3.5-turbo",  # 对话模型的名称
            #    messages=session,
            #    temperature=conf().get('temperature', 0.5),  # 值在[0,1]之间，越大表示回复越具有不确定性
                #max_tokens=4096,  # 回复最大的字符数
                #top_p=1,
                #frequency_penalty=conf().get('frequency_penalty', 0.0),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
                #presence_penalty=conf().get('presence_penalty', 0.0),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
            #)
            #[{'role': 'system', 'content': '你是ChatGPT, 一个由OpenAI训练的大型语言模型, 你旨在回答并解决人们的任何问题，并且可以使用多种语言与人交流。'}, {'role': 'user', 'content': '来首诗'}]
            #print(session)
            #[{"role": "system", "content": ''}, {"role": "user", "content": '来首诗'}]
            response=openai.ChatCompletion.create(model='gpt-3.5-turbo', temperature=0.05, messages=session)
            #logger.info("[ChatGPT] reply={}, total_tokens={}".format(response.choices[0]['message']['content'], response["usage"]["total_tokens"]))
